...
The solution to the model, its 'equilibrium', will pin down values for the endogenous variables of the model for a given set of parameters and exogenous variables. Thus, the endogenous variables, e.g. unemployment, are determined within the system, while exogenous variables are determined outside of the system and often reflect policy variables, such as the central bank interest rate or government spending. The equations connect the variables of the system. They typically express:
...
-   choose a set of numerical parameter values (e.g. a marginal propensity to consume of $c_1=0.8$)

-   choose (arbitrary but non-zero) initial values for the endogenous variables (e.g. $C=Y=1)$
...
Consider a two-dimensional simultaneous system represented by a simple Keynesian goods market model. We can use this model to make a theoretical prediction of how an increase in government spending affects aggregate output. 
...
Aggregate demand is composed of consumption $C$ and government spending $G_0$. The latter is assumed to be exogenous and will be treated as a causal driver of changes in aggregate income and consumption. Goods market equilibrium requires aggregate demand to be equal to aggregate income $Y$ (first equation). Consumption is assumed to be determined by an autonomous component $c_0$ and a marginal propensity to consume out of income $c_1$ (second equation).

Through the method of substitution, we can easily derive the analytical solution for equilibrium output: $Y^*=\frac{c_0+G_0}{1-c_1}$. With this solution, we can work out the predicted effect of an increase in government spending on output: $\frac{\partial Y^*}{\partial G_0}=\frac{1}{1-c_1}$.

We are interested in how an increase in government spending from $G_0=5$ to $G_0=6$ affects the solution of the system, especially equilibrium output. To solve the model numerically, suppose the parameters of the consumption function are given by $c_0=3$ and $c_1=0.8$. The code below shows how to find the solution via simulation.
...
#Create vector with exogenous value for government spending that will change across scenarios 
G0=vector(length=S)
G0[1]=5
G0[2]=6
...
# Verify solutions for Y using the analytical solution 
(c0+G0[])/(1-c1)
```
...
-   place these equations inside a *for loop*
    ...
    -   the loop says: repeat the segment of code inside the curly brackets 1000 times

-   nest the loop that solves the system in an outer loop that loops through different parameterisations (here for different values of $G_0$)
...
When does the method of iteration fail to provide a solution? In the following code, everything is the same with the only difference that the marginal propensity to consume is now larger than unity ($c_1=1.2$). Now the numerical simulation fails to find the correct solution. This is because with $c_1>1$, the so-called Keynesian equilibrium condition is violated and the equilibrium happens to be unstable. We will discuss in @sec-stab_analysis the issue of stability and how to analyse it formally. At this point, we simply note that the method of iterations requires static models to be stable to yield equilibrium solutions.
...
Consider now a dynamic version of the Keynesian goods market model that was proposed by Paul @Samuelson1939. In this model, investment ($I$) is endogenous and reacts to the change in consumption. Consumption ($C$) responds to changes in income ($Y$) with a lag:
...
# Set period in which shock or shift will occur
s=15
...
G0[2,s:Q]=6     # scenario: permanent increase in government spending from G0=5 to G0=6 from period s=15 onwards
...
# Set the period in which a shock or shift will occur
s = 15
...
# Set parameter values for different scenarios
G0[1, s:Q] = 6  # scenario: permanent increase in government spending from G0=5 to G0=6 from period s=15 onwards
...
axis(side = 4, at=pretty(range(I[1, 1:100])), tcl = 0.5)  
...
- add exogenous investment $I_0$ to aggregate demand
- set $I_0 = 2$
- create a third scenario where $I_0$ increases to $I_0=5$
...
G0=vector(length=S)
G0[]=5 # set G0=5 across all scenarios
G0[2]=6
# adding investment spending
I0=vector(length=S)
I0[]=2  # set I0=2 across all scenarios
I0[3]=5 # set I0=5 in third scenario
...
for (i in 1:S){
  for (iteration in 1:1000){
  Y = C + G0[i] + I0[i]           # add investment to aggregate demand
  C = c0 + c1*Y
  } # close iterations loop
...
barplot(Y_eq, ylab="Y", main="Output", names.arg=c("Baseline", "Increase in G0", "Increase in I0"))
...
I0 = np.zeros(S)
I0[:] = 2 # set I0=2 across all scenarios
I0[2] = 5 # set I0=5 in third scenario
...
# Bar chart of different scenarios for output
scenario_labels = ["Baseline", "Increase in G0", "Increase in I0"]
...
Sometimes, dynamic economic models are written in continuous time where the time increment is assumed to be infinitesimally small. Consider, for example, a continuous-time version of a Keynesian goods market model:
...
$$
Y(t+\Delta t)=Y_t + \dot{Y} \Delta t =Y_t + [k(C + G_0 - Y)]\Delta t.
...
#Solve this system recursively based on the initialisation
  for (t in 2:Q){
    ...
    Y[1,t] = Y[1,t-1] + delta*(k*(C[1,t-1] + G0[1,t-1] - Y[1,t-1]))
    C[1,t] = c0 + c1*Y[1,t]
...
for t in range(1, Q):
    for iterations in range(1000):
        Y[0, t] = Y[0, t - 1] + delta * (k * (C[0, t - 1] + G0[0, t - 1] - Y[0, t - 1]))
        C[0, t] = c0 + c1 * Y[0, t]
