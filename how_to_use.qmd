...
The website starts off with a general introduction to the numerical simulation of economic models (@sec-how-to-simulate). After that, it jumps right into a series of macroeconomic models. These models are broadly grouped into static and dynamic models. In static models, time plays no role and all variables adjust instantaneously. By contrast, dynamic models characterise the adjustment of variables over time.

The first set of models moves from static models of price and output determination (neoclassical, Keynesian, and Post-Keynesian) to models of income distribution and growth, while the second set is grouped into dynamic models of growth and distribution theories, business cycles, inflation and monetary policy, and more general modern modelling frameworks. Within each thematic group, models are ordered roughly chronologically. The model entries are largely self-contained and can be read independently of each other.
...
#Y=C+I
#C=c0+c1Y
# where Y is output, C is consumption, and I is exogenous investment
...
a) Write a loop which calculates $x_i=\frac{1}{i^2}$, where $i=1,2,..,10$, and saves each element $x_i$ in a vector.
...
# Option 2: set range of loop from 0 to 9, and add 1 in the index on the RHS
for i in range(0, 10):  
    x[i] = 1 / ((i+1)**2)
...
b) Write a second loop which calculates the cumulative sum $X_k= \sum^{k}_{i=1}x_i$, where $x_i=\frac{1}{i^2}$ and $k=1,.., 10$, and save each element $X_i$ in a vector. Combine both vectors in a matrix.
...
for (i in 1:10){
  x[i] = 1/i^2 
  if (i == 1) {
    X[i] <- x[i]            #  initialise cumulative sum
  } else {
    X[i] <- X[i - 1] + x[i] 
  }
}
