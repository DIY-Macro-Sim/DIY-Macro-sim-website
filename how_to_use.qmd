...
All simulation codes are written in the open-source programming language *R*. A Python extension is under construction and currently available for chapters 1 - 10. To be able to manipulate the codes on this platform on your own machine, you first need to [download and install *R* and *RStudio*](https://rstudio-education.github.io/hopr/starting.html). For Python, there are different options. One of them is to download and install *Spyder* via the [Anaconda Python distribution](https://www.anaconda.com/download). *Spyder* provides an interface for *Python* (like *RStudio* for *R*). If you install it via *Anaconda*, it will install *Python* automatically.
