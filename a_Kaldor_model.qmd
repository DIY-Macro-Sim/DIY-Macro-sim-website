...
axis(side = 4, at=pretty(K[1, Tmin:Tmax]), tcl = 0.5)
...
To analyse the dynamic properties of this discrete-time version of the @Kaldor1940 model, we first reduce it to a two-dimensional system in $Y_t$ and $K_t$. Substitution of @eq-kaldor_S and @eq-kaldor_I into @eq-kaldor_Y and @eq-kaldor_K yields: 
...
$$
K_{t} = (1-\delta)K_{t-1} + \sigma Y^E + \gamma \left(\frac{\sigma Y^E}{\delta} - K_{t-1} \right) + \arctan(Y_{t-1}- Y^E).
$$ {#eq-kaldor_K_reduc}
...
so that the eigenvalues will be complex if $tr(J)^2-4 det(J)<0$, which can also be written as $(J_{11}-J_{22})^2 + 4J_{12}J_{21}<0$, where $J_{ii}$ are the elements of the Jacobian matrix.^[See also @Stockhammer2019 on this condition for cycles.] Thus, the condition for cycles becomes:
