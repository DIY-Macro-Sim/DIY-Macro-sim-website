# A Conflict Inflation Model {#sec-inflation}
...
for (i in 1:S){
  ...
  for (t in 2:Q){
    ...
    r_T[i, t] = theta_T[i, t]/(1 + theta_T[i, t])
...
axis(side = 4, at=pretty(omega[2, Tmin:Tmax]), tcl = 0.5)
...
axis(side = 4, at=pretty(omega[3, Tmin:Tmax]), tcl = 0.5)
mtext(expression(Omega), side = 4, line = 3)
legend("topright", legend=c(expression(hat(p)),expression(hat(w)), expression(Omega)),
       lty=1:3, cex=0.8, bty = "n", y.intersp=0.8)
```
...
axis(side = 4, at=pretty(omega[4, Tmin:Tmax]), tcl = 0.5)
mtext(expression(Omega), side = 4, line = 3)
legend("topright", legend=c(expression(hat(p)),expression(hat(w)), expression(Omega)),
       lty=1:3, cex=0.8, bty = "n", y.intersp=0.8)
```
...
| Profit share of final output sector           | $$ \pi = \frac{\Pi}{pVA} $$                | $$ \pi = \frac{pY - wL - p^E E}{pY - p^E E} = 1 - \frac{\Omega}{1 - \delta\epsilon} $$ | Ambiguous                                                  |
...
into the term inside the square brackets yields:
