...
Table 1 reports the parameterisation used in the simulation. Besides a baseline (scenario 1), we consider a rise in the output ceiling $Y^C$ in scenario 2, an increase in the investment floor $I_0 - D$ in scenario 3, and finally a decrease in the accelerator coefficient $\beta$ (scenario 4).
...
| 2: rise in output ceiling ($Y^C$)               | 0   | 0.75    | 10    | 1.25    | 100   | 10  |
...
Then shift @eq-hicks_C_mod one period back and substitute into @eq-hicks_X to obtain:
...
@eq-hicks_X_mod and @eq-hicks_I_mod2 are a two-dimensional dynamic system in $X_t$ and $I_t$. 
...
To assess stability, we need to check whether the modulus of the complex eigenvalue $R$ is inside the unit circle, i.e. $R<1$. For a two-dimensional system with complex eigenvalues, we have $R=\sqrt{det(J)}$. Thus, the stability condition for the Hicks model is $\beta <1$.
