...
Consider the model by @Samuelson1939 discussed in @sec-how-to-simulate: 
$$
...
We can analyse the properties of the Samuelson model more generally by writing the condition for complex eigenvalues as $c_1 < \frac{4\beta}{(1+\beta)^2}$. A condition for stability can be derived by making use of the fact that $R=\sqrt{h^2+m^2}= \sqrt{ \left( \frac{-b}{2} \right) ^2 + \left(\frac{\sqrt{4c-b^2}}{2}i \right)^2} = \sqrt{c}$. Applied to the modulus of the Samuelson model, this yields $R=\sqrt{\beta c_1}$. Thus, the stability condition is $\beta c_1<1$.

The following code generates a plot that displays these two conditions in the $(\beta, c_1)$-space:
...
curve(cyc, from = 0, to = 5, col = 1, xlab=expression(beta), ylab=expression(c[1]) , main="",
      lwd=1.5, n=10000, ylim=range(0,2))
