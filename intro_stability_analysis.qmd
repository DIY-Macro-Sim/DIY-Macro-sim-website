...
If $a_1\neq1$, we can make use of the fact that the term $a_0\sum^{t-1}_{i=0} a_1^i$ is the sum of a geometric series which yields $a_0\frac{(1-a_1^t)}{1-a}$.

Thus, the solution takes the form:

$$
y_t = \frac{a_0(1-a_1^t)}{1-a} + a_1^ty_0 =\frac{a_0}{1-a} + a_1^t\left(y_0 - \frac{a_0}{1-a}\right).
$$
...
To see the last point, observe that in $\frac{y_{2t}}{y_{1t}}=\frac{\frac{1}{3}c_{1}2^t + 0.4c_{2}1^t}{c_{1}2^t + c_{2}1^t}$ the first terms in the numerator and denominator, respectively, quickly dominate the second terms as $t \rightarrow \infty$. Thus, $\frac{y_{2t}}{y_{1t}}$ will approach $\frac{1}{3}$ as $t \rightarrow \infty$.
...
## Compute solution manually for t=10 and compare with simulated solution
t=10
...
Consider the model by @Samuelson1939 discussed previously: $$
...
By De Moivre's theorem, we have $R(\cos\theta \pm \sin\theta \times i)^t=R^t(\cos\theta t \pm \sin\theta t \times i)$. Thus, the solution to a dynamic system that exhibits complex eigenvalues will be of the form: $$
...
An example for the stability analysis of a simple nonlinear system can be found in @sec-malthus.
